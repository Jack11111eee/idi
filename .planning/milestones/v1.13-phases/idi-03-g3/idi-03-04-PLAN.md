...
must_haves:
  truths:
    ...
    - "phase5_awaiting_tier 视图:#tier-modal 档位模态弹出(两选项「宽松」「严格」+ 各一句话说明,§8.2 口径);选择 → POST /api/checks/tier → 拉新(session 拉新后 state 仍 awaiting_tier 但已有 tier 签名 → 用户点「开始自检」按钮 POST /api/checks/start 或选档后直接起——按 D-P3-11 不自动起检,呈现「开始自检」按钮);已有 check 报告时的档位恢复呈现归 phase5_checking 视图按 selfcheck.tier 处理(awaiting_tier + 已有报告为结构性不可达态——有报告 ⇒ derive_state ≥ phase5_checking,§7.4 行 5/6 互斥)← DATA-04 / D-P3-11 / ROADMAP 判据 3"
...
<task type="auto">
  ...
  <behavior>
    - phase5_awaiting_tier + 无 check 报告(选档未落或已落未起检):#tier-modal 弹出,两选项各一句话说明(宽松 = 一次核查+修复+追加 PASS 即止;严格 = 循环核查至零问题轮 PASS,纯 P2 轮交用户裁决);点选 → POST /api/checks/tier → 200 → 拉新后呈「开始自检」按钮(不自动起验,D-P3-11)。(注:「awaiting_tier 且已有 check 报告」是结构性不可达态——有报告 ⇒ derive_state ≥ phase5_checking,§7.4 行 5/6 互斥;已有报告时的档位恢复呈现归 phase5_checking 视图按 selfcheck.tier 处理,不在本分支)
